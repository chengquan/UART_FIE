	...
	int Kx, int Ky, int Sx, int Sy, int Px, int Py, int data_width, int mp_mode)
{
	...
	switch (mp_mode) //mode0 = Booth; mode1 = HPS; mode2 = BSC; mode3 = LPC
	{
		...
		case 3: Tin_L = Tin * (unsigned int)(pow(2,(MAX_DAT_DW / data_width))); break;
...
void Mul_Conv_Bias_Soft(struct Conv_Cfg conv_cfg, int relu_en, struct Mapped_Feature* feature_in, struct Mapped_Weight* wt, struct Mapped_Feature* bias, struct Mapped_Feature* feature_out)
{
	...
	if ((feature_in->scale + wt->scale - feature_out->scale) < 0) {
		out_truncate = (feature_out->scale - feature_in->scale - wt->scale);
		//printf("��ʱ��out_truncateΪ%d,feature_out->scaleΪ%d��feature_in->scaleΪ%d,wt->scaleΪ%d\n", out_truncate, feature_out->scale, feature_in->scale, wt->scale);
	}
	else {
		...
		for (int n = 0; n < conv_cfg.CHout_Split_Times; n = n + 1)
			for (int k = 0; k < conv_cfg.Hout_Split_Times; k = k + 1)
			{
				...
				FPGA_Conv_single_time(Tin_factor, Tin_L, CH_in_res_Tin_div_Tout_minus1, shift_sacle,
					...
					((unsigned int)feature_in->payload) + feature_in->line_stride * line_offset_in, feature_in->surface_stride, feature_in->line_stride, feature_in->scale,
					((unsigned int)wt->payload) + conv_cfg.wt_size_in_bytes / conv_cfg.CHout * conv_cfg.out_ch_slice * n, conv_cfg.wt_size_in_bytes / conv_cfg.CHout * ch_out_single * 8 / (Tout * in_dw), wt_num_div_Tin / conv_cfg.CHout * ch_out_single, wt->scale,
					((unsigned int)feature_out->payload) + feature_out->line_stride * line_offset_out + feature_out->surface_stride * n * (conv_cfg.out_ch_slice / Tout), feature_out->surface_stride, feature_out->line_stride, feature_out->scale,//conv_out_scale
		...
		for (int k = 0; k < conv_cfg.Hout_Split_Times; k = k + 1)
			for (int n = 0; n < conv_cfg.CHout_Split_Times; n = n + 1)
			{
				...
				FPGA_Conv_single_time(Tin_factor, Tin_L, CH_in_res_Tin_div_Tout_minus1, shift_sacle,
					...
					conv_cfg.pad_x, pad_y_single, relu_en, out_dw,
					((unsigned int)feature_in->payload) + feature_in->line_stride * line_offset_in, feature_in->surface_stride, feature_in->line_stride, feature_in->scale,
					((unsigned int)wt->payload) + conv_cfg.wt_size_in_bytes / conv_cfg.CHout * conv_cfg.out_ch_slice * n, conv_cfg.wt_size_in_bytes / conv_cfg.CHout * ch_out_single * 8 / (in_dw * Tout), wt_num_div_Tin / conv_cfg.CHout * ch_out_single, wt->scale,
					((unsigned int)feature_out->payload) + feature_out->line_stride * line_offset_out + feature_out->surface_stride * n * (conv_cfg.out_ch_slice / Tout), feature_out->surface_stride, feature_out->line_stride, feature_out->scale,//conv_out_scale
	...
	int out_width, int out_height, int dat_buf_num, int cdma_dat_reuse, int cdma_wt_reuse)
{
	...
	//printf("Tin_factor is %x\n",CSB_Read(2));
	CSB_Write(3, dat_buf_num);
	//printf("dat_buf_num is %x\n",CSB_Read(3));
	CSB_Write(4, Win);
	CSB_Write(5, Hin);
	CSB_Write(6, Win * Hin);
	...
	while (CSB_Read(37) != 1)
	{
		...
		count++;
		delay_1ms(10);
		if(count == 200)//if cannot be jumped within 2s.
		{
			printf("Conv crashed due to the error config CSR.\r\n");
	        printf("print acc regs for error ana.\n");
            for(int i=0;i<41;i++)
            {
                printf("reg %d is %08x\n",i,CSB_Read(i));
            }
            printf("print pool regs for error ann.\n");
            for(int i=128;i<172;i++)
            {
                printf("reg %d is %08x\n",i,CSB_Read(i));
            }
			printf("error CSR record end.\r\n");
		}
		//printf("wait for wdma done\r\n");
