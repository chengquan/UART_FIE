...
int main(void)
{
    ...
    printf("MISA: 0x%lx\r\n", misa);
    print_misa();
    printf("Hello, This is Lenet INT48 test for Microsemi-07AA3 22/09/05!!\n");
    FIE_cmd_init();
    uart0_init();
    mnist_init();
    gpio_enable_output(GPIOA,1<<0);
    while(1)
    {
        FIE_cmd_fsm();
        led_interval++;
        if(led_interval == 800000)
        {
           gpio_write(GPIOA,1<<0,led_sts);
           led_interval = 0;
           led_sts = ((led_sts == 0) ? 1:0);
        }
    }
    return 0;
}
