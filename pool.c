...
void FPGA_Pool(struct Pool_Cfg pool_cfg, int pooling_method, struct Mapped_Feature* feature_in, struct Mapped_Feature* feature_out)
{
	...
	CSB_Write(pool_reg_bias + 3, (uint32_t)(feature_in->payload));//feature_in_base_addr
	...
	CSB_Write(pool_reg_bias + 6, (uint32_t)(feature_out->payload));//feature_out_base_addr
...
//Kick of the run
	...
	while (CSB_Read(pool_reg_bias + 1) != 1)
	{
		...
		{
			printf("Pool crashed due to the error config CSR.\r\n");
